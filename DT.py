...
# 为两个位点分别进行模型训练和保存
for i in range(2, 4):
    print(f"Model training for target column: {i}")
    targets = df.iloc[:, i].values
    ...
    # 网格搜索初始化
    grid_search = GridSearchCV(DecisionTreeRegressor(), param_grid, cv=kfold, scoring='r2')
    grid_search.fit(X_train, y_train)

    # 最佳模型保存
    best_model = grid_search.best_estimator_
    joblib.dump(best_model, f'decision_tree_trained_model_point_{i}.pkl')

    # 输出最佳参数和评估指标
    print(f"Best Hyperparameters for target {i}: {grid_search.best_params_}")
    ...
    df_results[f'Original_Target_{i}'] = y_test
    df_results[f'Predicted_Target_{i}'] = y_pred

# 将结果保存到 Excel 表
df_results.to_excel('decision_tree_prediction_results.xlsx', index=False)
