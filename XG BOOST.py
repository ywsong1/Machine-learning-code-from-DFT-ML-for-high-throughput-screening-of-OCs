...
# 加载数据
df = pd.read_excel('Gibbs energy.xlsx', skiprows=1)  # 跳过第一行标签
...
for i in range(2, 5):
    ...
    joblib.dump(best_model, f'xgboost_trained_model_point_{i}.pkl')
...
# 将结果保存到 Excel 表
df_results.to_excel('xgboost_prediction_results.xlsx', index=False)
