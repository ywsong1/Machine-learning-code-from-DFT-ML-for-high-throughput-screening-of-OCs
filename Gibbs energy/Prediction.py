...
gbdt_3 = joblib.load('trained_model_point_4.pkl')
# 读取新的化学式数据
df_new = pd.read_excel('Gibbs energy(test).xlsx', usecols=[1])
...
predictions_new_3 = gbdt_3.predict(features_new)
...
df_result = pd.DataFrame({
    ...
    'Prediction_Point_3': predictions_new_3
})
df_result.to_excel('Gibbs energy(test).xlsx', index=False)
